...
function renderPopular() {
  ...
  // Build full list: every product with its order count (0 if never ordered)
  const all = PRODUCTS.map(function (p) {
    return { id: p.id, name: p.name, count: tally[p.id] || 0 };
  });

  // Sort descending for top 5, ascending for bottom 5
  const sorted     = all.slice().sort(function (a, b) { return b.count - a.count; });
